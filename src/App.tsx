...
import { fetchAppState, saveAppState } from './api/stateApi';
...
type NotificationSettings = { email: boolean; push: boolean; dashboardAlerts: boolean; emailAlerts: boolean };
type PrivacySettings = { profilePublic: boolean };

const DEFAULT_THEME_PREFERENCE: ThemePreference = 'light';
const DEFAULT_NOTIFICATION_SETTINGS: NotificationSettings = { email: true, push: false, dashboardAlerts: true, emailAlerts: true };
const DEFAULT_PRIVACY_SETTINGS: PrivacySettings = { profilePublic: true };
...
interface StoreContextType {
  ...
  addNotification: (
    ...
    notificationMeta?: { linkChatUserId?: string }
  ) => void;
...
export default function App() {
  ...
  const legacyThemePreference = (() => {
    const saved = localStorage.getItem('migratehub_theme_preference');
    return (saved === 'light' || saved === 'dark' || saved === 'system') ? (saved as ThemePreference) : DEFAULT_THEME_PREFERENCE;
  })();
  const legacyNotificationSettings = (() => {
    const saved = loadData('notificationSettings', DEFAULT_NOTIFICATION_SETTINGS);
    return { ...DEFAULT_NOTIFICATION_SETTINGS, ...(saved && typeof saved === 'object' ? saved : {}) };
  })();
  const legacyPrivacySettings = (() => {
    const saved = loadData('privacySettings', DEFAULT_PRIVACY_SETTINGS);
    return { ...DEFAULT_PRIVACY_SETTINGS, ...(saved && typeof saved === 'object' ? saved : {}) };
  })();

  const [themePreferenceByUser, setThemePreferenceByUserState] = useState<Record<string, ThemePreference>>(() => {
    const saved = loadData('themePreferenceByUser', {});
    return saved && typeof saved === 'object' && !Array.isArray(saved) ? saved : {};
  });
  const [notificationSettingsByUser, setNotificationSettingsByUserState] = useState<Record<string, NotificationSettings>>(() => {
    const saved = loadData('notificationSettingsByUser', {});
    return saved && typeof saved === 'object' && !Array.isArray(saved) ? saved : {};
  });
  const [privacySettingsByUser, setPrivacySettingsByUserState] = useState<Record<string, PrivacySettings>>(() => {
    const saved = loadData('privacySettingsByUser', {});
    return saved && typeof saved === 'object' && !Array.isArray(saved) ? saved : {};
  });

  const normalizeNotificationSettings = (value: any): NotificationSettings => ({
    ...DEFAULT_NOTIFICATION_SETTINGS,
    ...(value && typeof value === 'object' ? value : {}),
  });
  const normalizePrivacySettings = (value: any): PrivacySettings => ({
    ...DEFAULT_PRIVACY_SETTINGS,
    ...(value && typeof value === 'object' ? value : {}),
  });
  const normalizeThemePreference = (value: any): ThemePreference => {
    if (value === 'light' || value === 'dark' || value === 'system') return value;
    return DEFAULT_THEME_PREFERENCE;
  };
  const normalizeUserMap = <T,>(value: any, normalizer: (entry: any) => T): Record<string, T> => {
    if (!value || typeof value !== 'object' || Array.isArray(value)) return {};
    const normalized: Record<string, T> = {};
    Object.entries(value).forEach(([userId, entry]) => {
      if (userId) normalized[userId] = normalizer(entry);
    });
    return normalized;
  };
  const mapLegacyValueToUsers = <T,>(allUsers: User[], value: T): Record<string, T> => {
    const mapped: Record<string, T> = {};
    allUsers.forEach((user) => {
      if (value && typeof value === 'object') {
        mapped[user.id] = { ...(value as Record<string, any>) } as T;
      } else {
        mapped[user.id] = value;
      }
    });
    return mapped;
  };

  const getThemePreferenceForUser = (userId?: string | null): ThemePreference => {
    if (!userId) return legacyThemePreference;
    return normalizeThemePreference(themePreferenceByUser[userId] ?? legacyThemePreference);
  };
  const getNotificationSettingsForUser = (userId?: string | null): NotificationSettings => {
    if (!userId) return legacyNotificationSettings;
    return normalizeNotificationSettings(notificationSettingsByUser[userId] ?? legacyNotificationSettings);
  };
  const getPrivacySettingsForUser = (userId?: string | null): PrivacySettings => {
    if (!userId) return legacyPrivacySettings;
    return normalizePrivacySettings(privacySettingsByUser[userId] ?? legacyPrivacySettings);
  };

  const themePreference = getThemePreferenceForUser(currentUser?.id);
  const notificationSettings = getNotificationSettingsForUser(currentUser?.id);
  const privacySettings = getPrivacySettingsForUser(currentUser?.id);

  const setThemePreference = (pref: ThemePreference) => {
    if (!currentUser?.id) return;
    setThemePreferenceByUserState((prev) => ({ ...prev, [currentUser.id]: normalizeThemePreference(pref) }));
  };
  const setNotificationSettings = (settings: NotificationSettings) => {
    if (!currentUser?.id) return;
    setNotificationSettingsByUserState((prev) => ({ ...prev, [currentUser.id]: normalizeNotificationSettings(settings) }));
  };
  const setPrivacySettings = (settings: PrivacySettings) => {
    if (!currentUser?.id) return;
    setPrivacySettingsByUserState((prev) => ({ ...prev, [currentUser.id]: normalizePrivacySettings(settings) }));
  };
  ...
  useEffect(() => {
    const hydrateFromServer = async () => {
      ...
      if (data) {
        const incomingUsers: User[] = Array.isArray(data.users) ? data.users : users;
        ...
        if (Array.isArray(data.libraryDocuments)) setLibraryDocuments(data.libraryDocuments);
        ...
        if (typeof data.selectedCurrency === 'string') setSelectedCurrency(data.selectedCurrency);
        if (Array.isArray(data.visaTypes)) setVisaTypes(data.visaTypes);
        if (Array.isArray(data.documentTypes)) setDocumentTypes(data.documentTypes);
        if (Array.isArray(data.pipelineStages)) setPipelineStages(data.pipelineStages);
        if (Array.isArray(data.gateways)) setPaymentGateways(data.gateways);
        if (Array.isArray(data.notifications)) setNotifications(data.notifications);
        if (data.notificationSettingsByUser && typeof data.notificationSettingsByUser === 'object') {
          setNotificationSettingsByUserState(normalizeUserMap(data.notificationSettingsByUser, normalizeNotificationSettings));
        } else if (data.notificationSettings && typeof data.notificationSettings === 'object') {
          const legacy = normalizeNotificationSettings(data.notificationSettings);
          setNotificationSettingsByUserState(mapLegacyValueToUsers(incomingUsers, legacy));
        }
        if (data.privacySettingsByUser && typeof data.privacySettingsByUser === 'object') {
          setPrivacySettingsByUserState(normalizeUserMap(data.privacySettingsByUser, normalizePrivacySettings));
        } else if (data.privacySettings && typeof data.privacySettings === 'object') {
          const legacy = normalizePrivacySettings(data.privacySettings);
          setPrivacySettingsByUserState(mapLegacyValueToUsers(incomingUsers, legacy));
        }
        if (data.themePreferenceByUser && typeof data.themePreferenceByUser === 'object') {
          setThemePreferenceByUserState(normalizeUserMap(data.themePreferenceByUser, normalizeThemePreference));
        } else if (typeof data.themePreference === 'string') {
          const legacy = normalizeThemePreference(data.themePreference);
          setThemePreferenceByUserState(mapLegacyValueToUsers(incomingUsers, legacy));
        }
        if (Array.isArray(data.email_templates)) setEmailTemplates(data.email_templates);
        if (data.emailjs_config && typeof data.emailjs_config === 'object') setEmailJSConfig(data.emailjs_config);
        if (Array.isArray(data.queued_admin_notifications)) setQueuedAdminNotifications(data.queued_admin_notifications);
      }
  ...
  useEffect(() => {
    ...
    saveData('themePreferenceByUser', themePreferenceByUser);
    saveData('notificationSettingsByUser', notificationSettingsByUser);
    saveData('privacySettingsByUser', privacySettingsByUser);
    // Legacy mirrors retained for compatibility with older backups/clients.
    saveData('themePreference', themePreference);
    ...
    const payload = {
      ...
      queued_admin_notifications: queuedAdminNotifications,
    };

    serverSyncTimerRef.current = window.setTimeout(() => {
      void saveAppState(payload);
    }, 800);
  }, [users, documents, messages, meetings, payments, agencies, libraryDocuments, destinations, currencies, selectedCurrency, visaTypes, documentTypes, pipelineStages, paymentGateways, notifications, themePreferenceByUser, notificationSettingsByUser, privacySettingsByUser, themePreference, notificationSettings, privacySettings, emailTemplates, emailJSConfig, queuedAdminNotifications, isReady, isServerStateLoaded]);
  ...
  useEffect(() => {
    ...
    const refreshedCurrentUser = users.find(u => u.id === currentUser.id);
    if (refreshedCurrentUser && refreshedCurrentUser !== currentUser) {
  ...
  useEffect(() => {
    if (!currentUser?.id) return;
    setThemePreferenceByUserState((prev) => (
      prev[currentUser.id]
        ? prev
        : { ...prev, [currentUser.id]: legacyThemePreference }
    ));
    setNotificationSettingsByUserState((prev) => (
      prev[currentUser.id]
        ? prev
        : { ...prev, [currentUser.id]: legacyNotificationSettings }
    ));
    setPrivacySettingsByUserState((prev) => (
      prev[currentUser.id]
        ? prev
        : { ...prev, [currentUser.id]: legacyPrivacySettings }
    ));
  }, [currentUser?.id]);
  ...
  const assignAgent = (userId: string, agentId: string) => {
    ...
    const clientName = client?.fullName || 'User';
    setUsers(prev => prev.map(u => u.id === userId ? { ...u, assignedAgentId: agentId, requestedAgentId: undefined } : u));
    addNotification(userId, "Agent Assigned", `Agent ${agentName} has been assigned to your case.`, 'AGENT_ASSIGNMENT', 'profile', {
      agentName: agentName
    }, { skipAdminBroadcast: true });
    addNotification(agentId, "New Client Assigned", `You have been assigned to client ${clientName}.`, 'AGENT_ASSIGNMENT', 'clients', {
      name: agentName,
    ...
    users
      ...
      .forEach(admin => {
        addNotification(admin.id, "Agent Assigned", `"${agentName}" has been assigned to "${clientName}".`, 'AGENT_ASSIGNMENT', 'clients', {
          agentName: agentName,
          clientName: clientName
        });
      });
    ...
    if (currentUser.role === 'ADMIN' || currentUser.role === 'SUPER_ADMIN') {
      showToast(`"${agentName}" has been assigned to "${clientName}".`);
    } else {
  ...
  const rejectAgentRequest = (userId: string) => {
    const client = users.find(u => u.id === userId);
    const requestedAgent = users.find(u => u.id === client?.requestedAgentId);
    setUsers(prev => prev.map(u => u.id === userId ? { ...u, requestedAgentId: undefined } : u));
    if (client) {
      addNotification(
        userId,
        "Agent Request Declined",
        requestedAgent
          ? `Your request for agent ${requestedAgent.fullName} was declined by ${currentUser.fullName}.`
          : `Your agent request was declined by ${currentUser.fullName}.`,
        'AGENT_ASSIGNMENT',
        'find-agent',
        {
          agentName: requestedAgent?.fullName || 'Selected Agent',
  ...
  const addNotification = (
    ...
    const baseNotification = {
      ...
      type,
      linkTab,
      linkChatUserId: notificationMeta?.linkChatUserId,
    ...
    const recipientSettings = getNotificationSettingsForUser(userId);
    if (recipientSettings.dashboardAlerts) {
    ...
    if (recipientSettings.emailAlerts && emailJSConfig.enabled && recipient) {
      if (recipient.role === 'USER' || recipient.role === 'AGENT') {
        ...
        switch (type) {
          ...
          case 'MESSAGE': templateId = 'new_message'; break;
        ...
        if (recipientSettings.emailAlerts && emailJSConfig.enabled) {
    ...
    if (!options?.skipAdminBroadcast && recipient && recipient.role !== 'ADMIN' && recipient.role !== 'SUPER_ADMIN') {
      const admins = users.filter(u => u.role === 'ADMIN' || u.role === 'SUPER_ADMIN');
      admins.forEach(admin => {
        if (admin.id !== userId) {
          const adminSettings = getNotificationSettingsForUser(admin.id);
          const adminNotif: Notification = {
            ...baseNotification,
            id: Math.random().toString(36).substr(2, 9),
            userId: admin.id
          };
          if (adminSettings.dashboardAlerts) {
            newNotifications.push(adminNotif);
          }
          
          // Daily summary logic for admins (simulated)
          if (adminSettings.emailAlerts && emailJSConfig.enabled) {
  ...
  const restoreSystem = (backup: any) => {
    try {
      ...
      const restoredUsers: User[] = Array.isArray(data.users) ? data.users : users;
      if (data.notificationSettingsByUser && typeof data.notificationSettingsByUser === 'object') {
        setNotificationSettingsByUserState(normalizeUserMap(data.notificationSettingsByUser, normalizeNotificationSettings));
      } else if (data.notificationSettings) {
        const legacy = normalizeNotificationSettings(data.notificationSettings);
        setNotificationSettingsByUserState(mapLegacyValueToUsers(restoredUsers, legacy));
      }
      if (data.privacySettingsByUser && typeof data.privacySettingsByUser === 'object') {
        setPrivacySettingsByUserState(normalizeUserMap(data.privacySettingsByUser, normalizePrivacySettings));
      } else if (data.privacySettings) {
        const legacy = normalizePrivacySettings(data.privacySettings);
        setPrivacySettingsByUserState(mapLegacyValueToUsers(restoredUsers, legacy));
      }
      if (data.themePreferenceByUser && typeof data.themePreferenceByUser === 'object') {
        setThemePreferenceByUserState(normalizeUserMap(data.themePreferenceByUser, normalizeThemePreference));
      } else if (data.themePreference) {
        const legacy = normalizeThemePreference(data.themePreference);
        setThemePreferenceByUserState(mapLegacyValueToUsers(restoredUsers, legacy));
      }
...
function Sidebar({ isOpen, setIsOpen }: { isOpen: boolean, setIsOpen: (v: boolean) => void }) {
  ...
  const menuItems = [
    ...
    { id: 'agencies', label: 'Agencies', icon: Building2, roles: ['SUPER_ADMIN', 'ADMIN'] },
...
function NotificationsPage() {
  const { notifications, currentUser, markNotificationAsRead, clearAllNotifications, setActiveTab, setSelectedChatContactId } = useStore();
  const myNotifications = notifications.filter(n => n.userId === currentUser.id);
  const hasNotifications = myNotifications.length > 0;

  const handleNotificationClick = (n: Notification) => {
    markNotificationAsRead(n.id);
    if (n.linkTab === 'messaging' && n.linkChatUserId) {
      setSelectedChatContactId(n.linkChatUserId);
    }
    if (n.linkTab) {
        ...
        <button
          ...
          className={cn(
            ...
            hasNotifications
              ...
              : "bg-secondary/40 text-muted-foreground border-border cursor-not-allowed opacity-70"
          )}
        >
          Clear All
        </button>
