...
export default function Meetings() {
  const { meetings, currentUser, users, updateMeetingStatus, updateMeeting, addMeeting, deleteMeeting, setMeetings, showToast } = useStore();
  ...
  const handleOpenModal = (meeting?: Meeting) => {
    if (meeting) {
      ...
      setMeetingTime(date.toTimeString().split(' ')[0].slice(0, 5));
  ...
  const handleSchedule = (e: React.FormEvent) => {
    ...
    const startTime = `${meetingDate}T${meetingTime}:00Z`;
...
function MeetingCard({ meeting, currentUser, users, updateStatus, onEdit, onDelete }: any) {
    ...
    <div className="bg-card p-6 rounded-3xl border border-border shadow-sm hover:shadow-xl transition-all group hover:scale-[1.02] active:scale-[0.99]">
      ...
      <div className="mt-6 pt-5 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          {isPast && (
            <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest opacity-50">Session Concluded</span>
          )}
        </div>
      </div>
    </div>
