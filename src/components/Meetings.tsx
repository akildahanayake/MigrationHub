...
export default function Meetings() {
  const { meetings, currentUser, users, updateMeetingStatus, updateMeeting, addMeeting, deleteMeeting, setMeetings, showToast, addNotification } = useStore();
  ...
  const upcomingMeetings = filteredMeetings.filter(m => new Date(m.startTime) > new Date() && m.status !== 'DECLINED');
  const pastMeetings = filteredMeetings.filter(m => new Date(m.startTime) <= new Date() || m.status === 'DECLINED');

  const [selectedUserId, setSelectedUserId] = useState('');
  const [selectedAgentId, setSelectedAgentId] = useState(currentUser.role === 'AGENT' ? currentUser.id : '');
  const [meetingDate, setMeetingDate] = useState('');
  const [meetingTime, setMeetingTime] = useState('');
  const [meetingTitle, setMeetingTitle] = useState('Consultation Session');
  const [meetingLink, setMeetingLink] = useState('https://zoom.us/j/demo');

  const toInputDate = (isoDateTime: string) => {
  ...
  const toInputTime = (isoDateTime: string) => {
    const d = new Date(isoDateTime);
  ...
  const handleOpenModal = (meeting?: Meeting) => {
    if (meeting) {
      ...
      setSelectedUserId(meeting.userId);
      setSelectedAgentId(meeting.agentId);
      setMeetingDate(toInputDate(meeting.startTime));
      setMeetingTime(toInputTime(meeting.startTime));
      setMeetingTitle(meeting.title);
      setMeetingLink(meeting.link);
    } else {
      setEditingMeeting(null);
      setSelectedUserId('');
      setSelectedAgentId(currentUser.role === 'AGENT' ? currentUser.id : '');
      setMeetingDate('');
      setMeetingTime('');
      setMeetingTitle('Consultation Session');
      setMeetingLink('https://zoom.us/j/demo');
    }
  ...
  const handleSchedule = (e: React.FormEvent) => {
    ...
    if (!agentId || !userId) {
      showToast("Please select both agent and client", "error");
      return;
    }

    const startTimeLocal = new Date(`${meetingDate}T${meetingTime}:00`);
    if (Number.isNaN(startTimeLocal.getTime())) {
      showToast("Please select a valid meeting date and time", "error");
      return;
    }
    const startTime = startTimeLocal.toISOString();
    ...
    if (editingMeeting) {
      updateMeeting(editingMeeting.id, {
        ...
        startTime,
        link: meetingLink,
        status: 'PENDING' // Reset to pending when edited/rescheduled
      });
    } else {
      addMeeting({
        agentId,
  ...
  const handleSessionOutcomeChange = (meetingId: string, sessionOutcome: NonNullable<Meeting['sessionOutcome']>) => {
    setMeetings(prev =>
      prev.map(m => {
        if (m.id !== meetingId) return m;

        let nextStatus = m.status;
        if (sessionOutcome === 'REQUEST_RESCHEDULE') nextStatus = 'RESCHEDULE_REQUESTED';
        if (sessionOutcome === 'SESSION_CONCLUDED') nextStatus = 'COMPLETED';
        if (sessionOutcome === 'NO_SHOW' || sessionOutcome === 'CANCELLED') nextStatus = 'DECLINED';

        return { ...m, sessionOutcome, status: nextStatus };
      })
    );

    if (sessionOutcome === 'REQUEST_RESCHEDULE') {
      const meeting = meetings.find(m => m.id === meetingId);
      if (meeting) {
        addNotification(
          meeting.agentId,
          'Past Session Reschedule Requested',
          `${currentUser.fullName} requested a reschedule for past meeting "${meeting.title}".`,
          'MEETING',
          'meetings',
          {
            title: meeting.title,
    ...
    showToast('Session outcome updated');
  };
  ...
  const availableClients = users.filter(u => {
    ...
    if (currentUser.role === 'ADMIN' || currentUser.role === 'SUPER_ADMIN') {
      ...
      return u.assignedAgentId === selectedAgentId;
    }
    return false;
  });
    ...
    <div className="space-y-8">
        ...
        <div className="space-y-6">
          ...
          {upcomingMeetings.map(meet => (
            <MeetingCard 
              ...
              onSessionOutcomeChange={handleSessionOutcomeChange}
              onEdit={() => handleOpenModal(meet)}
              onDelete={() => setMeetingToDelete(meet)}
            />
          ))}
      ...
      {showScheduleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-card rounded-[2.5rem] p-10 max-w-md w-full shadow-2xl border border-border animate-in zoom-in duration-300">
            <h3 className="text-2xl font-black text-foreground mb-6">{editingMeeting ? 'Edit Meeting' : 'Schedule Consultation'}</h3>
            <form onSubmit={handleSchedule} className="space-y-4">
              <div>
                <label className="block text-sm font-bold text-muted-foreground mb-1">Meeting Title</label>
                <input 
                  type="text" 
                  className="w-full px-4 py-2 bg-secondary/50 rounded-xl border border-border text-foreground outline-none font-bold transition-all" 
                  value={meetingTitle}
              ...
              <div>
                ...
                >
                  <option value="" className="bg-card">
                    {(currentUser.role === 'ADMIN' || currentUser.role === 'SUPER_ADMIN') && !selectedAgentId
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full px-4 py-2 bg-secondary/50 border border-border text-foreground rounded-xl outline-none transition-all" 
                  value={meetingLink}
              ...
              <div className="flex gap-3 mt-8">
                ...
                <button type="submit" className="flex-1 px-4 py-2 rounded-xl bg-primary text-primary-foreground font-bold hover:bg-primary/90 transition-all shadow-lg shadow-primary/20 active:scale-95">
      ...
      <ConfirmationModal 
        ...
        message={`Are you sure you want to delete the meeting "${meetingToDelete?.title}"?`}
        confirmLabel="Delete"
        variant="danger"
      />
    </div>
...
function MeetingCard({
  ...
  currentUser: User;
  users: User[];
  updateStatus: (meetingId: string, status: Meeting['status']) => void;
  onSessionOutcomeChange: (meetingId: string, sessionOutcome: NonNullable<Meeting['sessionOutcome']>) => void;
  onEdit: () => void;
  onDelete: () => void;
}) {
  const agent = users.find((u: User) => u.id === meeting.agentId);
  const user = users.find((u: User) => u.id === meeting.userId);
  const partnerId = currentUser.role === 'USER' ? meeting.agentId : meeting.userId;
  const partner = users.find((u: User) => u.id === partnerId);
  const showBothParticipants = currentUser.role === 'ADMIN' || currentUser.role === 'SUPER_ADMIN';
  const date = new Date(meeting.startTime);
  const isPast = date <= new Date();
  const isAdmin = currentUser.role === 'ADMIN' || currentUser.role === 'SUPER_ADMIN';
  const isAgent = currentUser.role === 'AGENT';
  const canManage = isAdmin || (isAgent && meeting.agentId === currentUser.id);
  const isPastSession = isPast || meeting.status === 'DECLINED' || meeting.status === 'COMPLETED';
  const selectedOutcome = meeting.sessionOutcome ?? 'SESSION_CONCLUDED';
  const outcomeOptions: { value: NonNullable<Meeting['sessionOutcome']>; label: string }[] =
    currentUser.role === 'USER'
      ? [
          { value: 'SESSION_CONCLUDED', label: 'Session Concluded' },
    ...
    <div className="bg-card p-6 rounded-3xl border border-border shadow-sm hover:shadow-xl transition-all group hover:scale-[1.02] active:scale-[0.99]">
      <div className="flex items-start justify-between">
        <div className="flex gap-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              <img 
                src={getProfilePic(showBothParticipants ? agent?.fullName : partner?.fullName, showBothParticipants ? agent?.photoUrl : partner?.photoUrl)} 
                className="w-5 h-5 rounded-full object-cover border-2 border-border shadow-sm" 
                alt="" 
              />
              <span className="text-xs text-muted-foreground font-black uppercase tracking-tight">
                {showBothParticipants
                  ? `${agent?.fullName || 'Unknown Agent'} with ${user?.fullName || 'Unknown User'}`
                  : `with ${partner?.fullName}`}
              </span>
            </div>
          </div>
        </div>
        <div className="flex flex-col items-end gap-2">
          ...
          {(canManage && !isPast) || (isPastSession && isAdmin) ? (
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <button 
                ...
                title={isPastSession ? "Delete Past Meeting" : "Delete Meeting"}
              >
                <Trash2 size={14} />
              </button>
            </div>
      ...
      <div className="mt-6 pt-5 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          {meeting.status === 'PENDING' && currentUser.role === 'USER' && !isPast && (
            <>
              <button 
          ...
          {meeting.status === 'ACCEPTED' && !isPast && (
            <>
              <a 
                href={meeting.link} 
                target="_blank" 
          ...
          {meeting.status === 'RESCHEDULE_REQUESTED' && canManage && !isPast && (
            <button 
              onClick={onEdit}
          ...
          {meeting.status === 'RESCHEDULE_REQUESTED' && currentUser.role === 'USER' && !isPast && (
          ...
          {meeting.status === 'PENDING' && canManage && !isPast && (
          ...
          {isPastSession && (
            <select
              ...
              onChange={(e) => onSessionOutcomeChange(meeting.id, e.target.value as NonNullable<Meeting['sessionOutcome']>)}
              className="px-3 py-2 bg-secondary/60 border border-border text-foreground rounded-xl text-[10px] font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-primary"
            >
