...
export default function Agencies() {
  const { agencies, setAgencies, showToast, currentUser } = useStore();
  const normalizedRole = String(currentUser.role || '').trim().toUpperCase();
  const isManager = normalizedRole === 'SUPER_ADMIN' || normalizedRole === 'ADMIN';
  ...
  const handleDeleteAgency = () => {
    if (!agencyToDelete || !isManager) return;
    setAgencies(agencies.filter(a => a.id !== agencyToDelete.id));
    showToast(`Agency "${agencyToDelete.name}" deleted successfully!`);
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="flex items-center gap-2">
                ...
                {isManager && (
                  <button
                    ...
                    className="p-2 text-muted-foreground hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/30 rounded-lg transition-all"
                    title="Delete Agency"
                  >
                    <Trash2 size={16} />
                  </button>
      ...
      <ConfirmationModal
        ...
        title="Delete Agency"
        message={`Are you sure you want to delete "${agencyToDelete?.name}"? This action cannot be undone.`}
