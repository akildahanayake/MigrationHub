...
export default function Profiles({ user: propUser }: { user?: any }) {
  ...
  const handleSave = () => {
    ...
    if (passwordData.newPassword || passwordData.confirmPassword) {
      ...
      nextData.password = passwordData.newPassword;
    } else {
      delete nextData.password;
    }

    updateUser(displayUser.id, nextData);
    setIsEditing(false);
    setPasswordData({ newPassword: '', confirmPassword: '' });
      ...
      <div className="bg-card rounded-[2.5rem] p-10 shadow-2xl border border-border flex flex-col md:flex-row items-center gap-10 relative transition-all">
        {isOwnProfile && (
          <div className="absolute top-10 right-10 flex gap-3">
            ...
            >
              {isEditing ? <><Save size={18} strokeWidth={3} /> Save Profile</> : <><Edit2 size={18} strokeWidth={3} /> Edit Details</>}
            </button>
          </div>
        )}
      ...
      {isOwnProfile && isEditing && (
        <div className="bg-card rounded-[3rem] p-10 shadow-xl border border-border transition-all">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-2">New Password</p>
              <input
                type="password"
                value={passwordData.newPassword}
                onChange={(e) => setPasswordData(prev => ({ ...prev, newPassword: e.target.value }))}
                placeholder="Enter new password"
                className="w-full text-sm font-black text-foreground bg-secondary border border-border rounded-xl px-3 py-2 focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all"
              />
            </div>
            <div>
              <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-2">Confirm Password</p>
              <input
                type="password"
