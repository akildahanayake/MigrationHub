...
export default function Dashboard() {
  ...
  const getPipelineProgressPercent = (status?: string) => {
    if (!status || pipelineStages.length === 0) return 0;
    const stageIndex = pipelineStages.indexOf(status);
    if (stageIndex < 0) return 0;
    return Math.round(((stageIndex + 1) / pipelineStages.length) * 100);
  };
  ...
  const renderUserStats = () => {
    ...
    const progressPercent = getPipelineProgressPercent(currentUser.applicationStatus);
    return (
        ...
        <div className="bg-card p-8 rounded-[2rem] border border-border shadow-xl flex flex-col justify-between transition-all hover:scale-[1.01] active:scale-[0.99] group relative overflow-hidden">
          ...
          <div className="mt-10 space-y-4 relative z-10">
            <div className="flex justify-between text-[11px] font-black text-muted-foreground items-center">
              ...
              <span className="bg-primary/10 text-primary px-2 py-1 rounded-lg border border-primary/20 font-black">{progressPercent}%</span>
            </div>
            <div className="bg-muted h-5 rounded-full overflow-hidden p-1 border border-border shadow-inner">
              <div className="bg-gradient-to-r from-primary via-primary to-indigo-400 h-full rounded-full shadow-[0_0_15px_rgba(79,70,229,0.3)] transition-all duration-1000 relative" style={{ width: `${progressPercent}%` }}>
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-7xl mx-auto space-y-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 bg-gradient-to-br from-indigo-600 via-primary to-violet-700 p-10 lg:p-14 rounded-[3rem] text-primary-foreground shadow-2xl shadow-primary/20 relative overflow-hidden group border border-white/10">
        ...
        <div className="relative z-10 space-y-3">
          {isPrivilegedUser && (
            <div className={cn(
              "inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border backdrop-blur-md mb-4 shadow-xl",
              healthChecked && isDbLive
              ...
              <span className={cn(
                ...
                healthChecked && isDbLive
                  ...
                  : "bg-rose-300 shadow-[0_0_12px_rgba(251,113,133,0.9)]"
              )} />
              <span className="text-[10px] font-black uppercase tracking-[0.25em] text-white/90">
                {healthChecked && isDbLive ? 'System Secure & Live' : 'System Offline'}
              </span>
            </div>
          )}
          ...
          <div className="text-white/70 font-black uppercase tracking-[0.3em] text-[10px] animate-in slide-in-from-left duration-700 delay-100 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-card rounded-[2.5rem] shadow-xl border border-border overflow-hidden transition-all hover:shadow-2xl">
            <div className="px-8 py-6 border-b border-border flex items-center justify-between bg-card/50 backdrop-blur-md">
              <div className="flex items-center gap-3">
                <div className="w-2 h-6 bg-primary rounded-full" />
                <h3 className="font-black text-foreground uppercase tracking-tight">Recent Activity</h3>
              </div>
              <button 
                onClick={() => setActiveTab('notifications')}
                className="text-xs font-black text-primary hover:text-primary/80 uppercase tracking-widest bg-primary/10 px-4 py-2 rounded-xl transition-all active:scale-95"
              >
                View All
              </button>
            </div>
            <div className="divide-y divide-border transition-colors">
              {myNotifications.slice(0, 5).length > 0 ? (
                myNotifications.slice(0, 5).map(n => (
                  <button 
                    key={n.id} 
                    onClick={() => handleNotificationClick(n)}
                    className="w-full text-left p-6 flex items-center gap-6 hover:bg-muted/50 transition-all group relative"
                  >
                    <div className={cn(
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-1 text-[9px] font-black uppercase tracking-tight mb-1">
                        <span className="text-primary">{n.senderRole?.replace('_', ' ')}</span>
                        <span className="text-muted-foreground opacity-30">|</span>
                        <span className="text-foreground">{n.senderName}</span>
                        <span className="text-muted-foreground opacity-30">|</span>
                        <span className="text-foreground truncate max-w-[100px]">{n.title}</span>
                        <span className="text-muted-foreground opacity-30">|</span>
                      ...
                      <p className="text-xs text-muted-foreground font-medium truncate">{n.message}</p>
                    </div>
                ...
                <div className="p-12 text-center">
                  <Clock className="mx-auto text-muted-foreground/30 mb-4" size={40} />
                  <p className="text-sm text-muted-foreground font-bold uppercase tracking-widest">No recent activity</p>
                </div>
              )}
            </div>
          </div>
              ...
              <div className="space-y-5">
                {(() => {
                  ...
                  const stageData = stages.map((stage, stageIndex) => {
                    const completedCount = clientUsers.filter(u => {
                      const userStageIndex = stages.indexOf(u.applicationStatus || '');
                      return userStageIndex >= stageIndex;
                    }).length;
                    const percentage = totalClients > 0 ? Math.round((completedCount / totalClients) * 100) : 0;
                    return { stage, count: totalClients, percentage };
                  });
          ...
          {currentUser.role === 'AGENT' && (
            <div className="bg-card p-6 rounded-2xl shadow-sm border border-border transition-colors">
              ...
              <div className="space-y-5">
                {(() => {
                  const myClients = users.filter(u => u.assignedAgentId === currentUser.id);
                  const totalMyClients = myClients.length;
                  const stages = pipelineStages;

                  const stageData = stages.map((stage, stageIndex) => {
                    const completedCount = myClients.filter(u => {
                      const userStageIndex = stages.indexOf(u.applicationStatus || '');
                      return userStageIndex >= stageIndex;
                    }).length;
                    const percentage = totalMyClients > 0 ? Math.round((completedCount / totalMyClients) * 100) : 0;
                    const count = totalMyClients;
                    return { stage, count, percentage };
                  });

                  if (totalMyClients === 0) return <p className="text-sm text-muted-foreground py-4 text-center">No active clients assigned.</p>;
