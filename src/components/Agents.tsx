...
export default function Agents({ mode }: AgentsProps) {
  const { users, currentUser, requestAgent, addUser, updateUser, deleteUser, setActiveTab, setSelectedChatContactId, destinations, showToast } = useStore();
  ...
  const agents = users.filter(u => u.role === 'AGENT' && (mode === 'manage' || u.approvalStatus !== 'PENDING'));
  ...
  const openEditAgent = (agent: User) => {
    ...
    setEditAgent({
      ...
      email: agent.email || '',
      phone: agent.phone || '',
      agencyName: agent.agencyName || currentUser.agencyName || 'Alpha Migration',
      licenseNumber: agent.licenseNumber || '',
      yearsExperience: Number(agent.yearsExperience || 0),
      countriesSupported: (agent.countriesSupported || []).join(', '),
      visasSupported: (agent.visasSupported || []).join(', '),
      bio: agent.bio || '',
    });
  ...
  const handleEditAgent = (e: React.FormEvent) => {
    ...
    updateUser(agentToEdit.id, {
      fullName: editAgent.fullName,
      email: editAgent.email,
      phone: editAgent.phone,
      agencyName: editAgent.agencyName,
      licenseNumber: editAgent.licenseNumber,
      yearsExperience: Number(editAgent.yearsExperience),
      countriesSupported: editAgent.countriesSupported.split(',').map(s => s.trim()).filter(Boolean),
      visasSupported: editAgent.visasSupported.split(',').map(s => s.trim()).filter(Boolean),
      bio: editAgent.bio,
    } as any);

    showToast(`"${editAgent.fullName}" updated successfully`, "success");
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="relative bg-card w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-border">
            <div className="p-8 border-b border-border flex items-center justify-between bg-card">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-black text-foreground tracking-tight">Edit Migration Agent</h3>
                  <p className="text-xs text-muted-foreground font-bold uppercase tracking-widest mt-0.5">Update Agent Profile</p>
                </div>
              </div>
              <button onClick={() => setIsEditModalOpen(false)} className="p-2 hover:bg-muted rounded-xl transition-all hover:rotate-90">
                <X size={20} className="text-muted-foreground" />
              </button>
            </div>

            <form onSubmit={handleEditAgent} className="p-8 overflow-y-auto max-h-[75vh] bg-card">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-black text-foreground uppercase tracking-widest mb-1.5 ml-1">Full Name</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={e => setEditAgent({...editAgent, fullName: e.target.value})}
                      />
                    </div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={e => setEditAgent({...editAgent, licenseNumber: e.target.value})}
                      />
                    </div>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        type="number"
                        className="w-full pl-10 pr-4 py-3 bg-background border border-border rounded-xl text-sm text-foreground focus:ring-2 focus:ring-primary transition-all outline-none font-bold"
                        value={editAgent.yearsExperience || ''}
                        onChange={e => setEditAgent({...editAgent, yearsExperience: parseInt(e.target.value) || 0})}
                      />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        type="text"
                        className="w-full pl-10 pr-4 py-3 bg-background border border-border rounded-xl text-sm text-foreground focus:ring-2 focus:ring-primary transition-all outline-none font-bold"
                        value={editAgent.countriesSupported}
                        onChange={e => setEditAgent({...editAgent, countriesSupported: e.target.value})}
                      />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        type="text"
                        className="w-full pl-10 pr-4 py-3 bg-background border border-border rounded-xl text-sm text-foreground focus:ring-2 focus:ring-primary transition-all outline-none font-bold"
                        value={editAgent.visasSupported}
                        onChange={e => setEditAgent({...editAgent, visasSupported: e.target.value})}
                      />
                    </div>
                  </div>
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredAgents.map(agent => (
          <AgentCard 
            ...
            onApprove={() => {
              updateUser(agent.id, { approvalStatus: 'APPROVED' });
              showToast(`"${agent.fullName}" approved successfully`, "success");
            }}
          />
        ))}
      </div>
      ...
      <ConfirmationModal 
        ...
        onConfirm={() => {
          if (agentToDelete) {
            deleteUser(agentToDelete.id);
            setAgentToDelete(null);
          }
        }}
        title="Remove Agent"
        message={`Are you sure you want to remove Agent ${agentToDelete?.fullName}? This will unassign all their clients.`}
...
function AgentCard({ agent, mode, currentUser, onSelect, onChat, onViewAnalytics, onDelete, onEdit, onApprove }: any) {
  ...
  const isPendingApproval = agent.approvalStatus === 'PENDING';
  ...
  return (
    <div className="bg-card rounded-2xl border border-border shadow-sm hover:shadow-md transition-all overflow-hidden flex flex-col group/card relative">
      {mode === 'manage' && isAdmin && (
        <div className="absolute top-4 right-4 hidden md:flex items-center gap-2 opacity-0 group-hover/card:opacity-100 transition-all z-10">
          <button
            onClick={onEdit}
            className="p-2 bg-background/80 backdrop-blur-sm text-muted-foreground hover:text-primary rounded-xl border border-border shadow-sm"
            title="Edit Agent"
          >
            <Pencil size={18} />
          </button>
          <button
            onClick={onDelete}
            className="p-2 bg-background/80 backdrop-blur-sm text-muted-foreground hover:text-vibrant-red rounded-xl border border-border shadow-sm"
            title="Remove Agent"
          >
            <Trash2 size={18} />
          </button>
      ...
      <div className="p-6 flex-1">
        ...
        <div className="space-y-2 mb-6">
          {mode === 'manage' && isPendingApproval && (
            <div className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest bg-vibrant-yellow/10 text-vibrant-yellow border border-vibrant-yellow/20 px-2.5 py-1 rounded-lg">
              <Clock size={12} />
              Pending Approval
            </div>
          )}
        ...
        {mode === 'market' ? (
          ...
          <>
            {isAdmin && isPendingApproval && (
              <button
                onClick={onApprove}
                className="px-3 py-2 bg-vibrant-green text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:opacity-90 transition-all active:scale-95"
              >
                Approve
              </button>
            )}
