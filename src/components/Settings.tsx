...
const Settings: React.FC = () => {
  ...
  const sections = [
    ...(currentUser.role === 'SUPER_ADMIN' || currentUser.role === 'ADMIN' ? [
      ...
      { id: 'email-config', label: 'EmailJS Config', icon: Mail },
    ] : []),
  ...
  const toggleSetting = (key: 'email' | 'push' | 'dashboardAlerts' | 'emailAlerts' | 'profilePublic') => {
    if (key === 'profilePublic') {
      setPrivacySettings({ profilePublic: !privacySettings.profilePublic });
    } else {
  ...
  return (
    <div className="space-y-10 max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-6 duration-700">
      ...
      <div className="flex flex-col lg:flex-row gap-12">
        ...
        {/* Content */}
        <div className="flex-1 bg-card rounded-[3rem] border border-border shadow-[0_32px_64px_-12px_rgba(0,0,0,0.14)] transition-all relative">
          <div className="p-10 md:p-14 transition-colors relative z-20 bg-card shadow-inner">
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    { key: 'dashboardAlerts', label: 'Dashboard Alerts', desc: 'Instant in-app notifications for messages and updates.' },
                    { key: 'emailAlerts', label: 'Email Alerts', desc: 'Send automated emails for each activity.' },
                  ].map((item) => (
                    <div key={item.key} className="flex items-center justify-between p-6 bg-card rounded-3xl border border-border transition-all hover:bg-card/80">
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          (notificationSettings as any)[item.key] ? 'bg-primary' : 'bg-muted-foreground/30'
                        )}
                      >
                        <div className={cn(
                          "absolute top-1 left-1 w-6 h-6 bg-white rounded-full transition-transform shadow-md",
                          (notificationSettings as any)[item.key] ? 'translate-x-6' : ''
                        )} />
                      </button>
                    </div>
                  ))}
            ...
            {activeSection === 'email-config' && (
              <div className="space-y-10 animate-in fade-in slide-in-from-right-4">
                <div className="bg-vibrant-blue p-8 rounded-[3rem] text-white shadow-2xl shadow-vibrant-blue/20 mb-8 transition-all overflow-hidden relative border-none">
                  <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -translate-y-1/2 translate-x-1/2 blur-3xl" />
                  <div className="relative z-10">
                    <div className="flex items-center gap-5 mb-4">
                      <div className="w-14 h-14 bg-white/20 backdrop-blur-xl rounded-[1.5rem] flex items-center justify-center border border-white/20 shadow-inner">
                        <Mail className="w-8 h-8 text-white stroke-[2.5px]" />
                      </div>
                      <div>
                        <h3 className="text-2xl font-black tracking-tight text-white">EmailJS Configuration</h3>
                        <p className="text-white/70 font-bold uppercase text-[10px] tracking-widest">External Email API Integration</p>
                      </div>
                    </div>
                    <p className="text-sm font-black text-white/80 leading-relaxed italic max-w-2xl">
                      Configure your EmailJS credentials to enable automated email notifications. You can find these in your EmailJS dashboard.
                    </p>
                  </div>
                </div>

                <div className="bg-card border-2 border-border rounded-[2.5rem] p-8 md:p-12 space-y-8">
                  <div className="flex items-center justify-between p-6 bg-muted/30 rounded-3xl border border-border mb-8">
                    <div>
                      <p className="font-black text-foreground uppercase tracking-tight text-sm">Enable EmailJS Integration</p>
                      <p className="text-xs text-muted-foreground font-medium mt-1">Globally enable or disable automated emails.</p>
                    </div>
                    <button
                      onClick={() => {
                        setEmailJSConfig({ ...emailJSConfig, enabled: !emailJSConfig.enabled });
                        showToast(`EmailJS ${!emailJSConfig.enabled ? 'Enabled' : 'Disabled'}`, 'success');
                      }}
                      className={cn(
                        "w-14 h-8 rounded-full transition-all relative ring-4 ring-transparent hover:ring-vibrant-blue/10",
                        emailJSConfig.enabled ? "bg-vibrant-blue" : "bg-muted"
                      )}
                    >
                      <div className={cn(
                        "absolute top-1 left-1 w-6 h-6 bg-white rounded-full transition-transform shadow-md",
                        emailJSConfig.enabled ? "translate-x-6" : ""
                      )} />
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] ml-1">Service ID</label>
                      <input 
                        type="text" 
                        placeholder="service_..."
                        value={emailJSConfig.serviceId}
                        onChange={(e) => setEmailJSConfig({ ...emailJSConfig, serviceId: e.target.value })}
                        className="w-full px-6 py-4 bg-card border-2 border-border text-foreground rounded-2xl font-black outline-none focus:ring-4 focus:ring-vibrant-blue/10 focus:border-vibrant-blue transition-all placeholder:text-muted-foreground/50"
                      />
                    </div>
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] ml-1">Template ID</label>
                      <input 
                        type="text" 
                    ...
                    <div className="md:col-span-2 space-y-3">
                      <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] ml-1">Public Key</label>
                      <input 
                        type="text" 
                        placeholder="user_..."
                  ...
                  <div className="flex justify-end pt-4 gap-4">
                    <button 
                      onClick={() => sendAdminDailySummary()}
                      className="px-8 py-3.5 bg-card border border-border text-foreground rounded-2xl font-black uppercase tracking-[0.1em] text-[11px] hover:bg-muted transition-all flex items-center gap-3 hover:scale-[1.02] active:scale-[0.98]"
                    >
                      <Mail size={18} />
                      Send Daily Summary ({queuedAdminNotifications.length})
                    </button>
                    <button 
                      onClick={() => showToast("EmailJS configuration saved", "success")}
                      className="px-8 py-3.5 bg-vibrant-blue text-white rounded-2xl font-black uppercase tracking-[0.1em] text-[11px] hover:bg-vibrant-blue/90 shadow-xl shadow-vibrant-blue/20 transition-all flex items-center gap-3 hover:scale-[1.02] active:scale-[0.98]"
                    >
                      <Check size={18} strokeWidth={4} />
                      Save Configuration
                    </button>
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between p-6 bg-card rounded-[2rem] border border-border shadow-sm">
                    ...
                    <button
                      ...
                      className={cn(
                        "w-14 h-8 rounded-full transition-all relative ring-4 ring-transparent hover:ring-primary/10",
                        privacySettings.profilePublic ? 'bg-primary' : 'bg-muted-foreground/30'
                      )}
                    >
                      <div className={cn(
                        "absolute top-1 left-1 w-6 h-6 bg-white rounded-full transition-transform shadow-md",
                        privacySettings.profilePublic ? 'translate-x-6' : ''
                      )} />
                    </button>
                  </div>
                  ...
                  {(currentUser.role === 'ADMIN' || currentUser.role === 'SUPER_ADMIN') && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <button 
                        ...
                        className="flex items-center justify-between p-6 bg-card hover:bg-background rounded-[2rem] transition-all border border-border group shadow-sm hover:shadow-xl hover:-translate-y-1"
                      >
                        <div className="flex items-center gap-5">
                          <div className="w-14 h-14 bg-primary/10 rounded-2xl flex items-center justify-center group-hover:bg-primary/20 transition-colors">
                            <Download className="w-6 h-6 text-primary stroke-[2.5px]" />
                          </div>
                          <div className="text-left">
                            <p className="font-black text-foreground uppercase tracking-tight text-sm">Full System Backup</p>
                            <p className="text-[10px] text-muted-foreground font-bold">Export all system data to JSON.</p>
                          </div>
                        </div>
                      </button>
                      ...
                      <div className="relative">
                        ...
                        <button 
                          ...
                          className="w-full h-full flex items-center justify-between p-6 bg-card hover:bg-background rounded-[2rem] transition-all border border-border group shadow-sm hover:shadow-xl hover:-translate-y-1"
                        >
                          <div className="flex items-center gap-5">
                            <div className="w-14 h-14 bg-primary/10 rounded-2xl flex items-center justify-center group-hover:bg-primary/20 transition-colors">
                              <Upload className="w-6 h-6 text-primary stroke-[2.5px]" />
                            </div>
                            <div className="text-left">
                              <p className="font-black text-foreground uppercase tracking-tight text-sm">Restore System</p>
                              <p className="text-[10px] text-muted-foreground font-bold">Import system data from backup.</p>
                            </div>
                          </div>
                        </button>
                      </div>
          ...
          <div className="px-8 py-6 bg-card border-t border-border flex flex-col sm:flex-row justify-end gap-6 items-center transition-colors relative z-10">
            <div className="flex items-center gap-4">
              ...
              <button 
                onClick={() => {
                  setLocalTheme('light');
                  setNotificationSettings({ email: true, push: false, dashboardAlerts: true, emailAlerts: false });
                  setPrivacySettings({ profilePublic: true });
                  showToast('Your preferences were restored to defaults');
                }}
              ...
              <button 
                ...
                onClick={() => {
                  ...
                  setTimeout(() => {
                    ...
                    showToast('Personal settings updated');
      ...
      <ConfirmationModal 
        ...
        message="CRITICAL ACTION: Are you absolutely sure you want to terminate your account? All documents and visa progress will be PERMANENTLY DELETED. This cannot be undone."
