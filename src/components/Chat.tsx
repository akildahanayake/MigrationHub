...
export default function Chat() {
  const { messages, setMessages, currentUser, users, selectedChatContactId, setSelectedChatContactId, addNotification } = useStore();
  ...
  const lastMessageTimestampByContact = useMemo(() => {
    ...
    contacts.forEach(contact => {
      const lastTs = messages
        ...
        .sort((a, b) => b - a)[0] || 0;
      map.set(contact.id, lastTs);
    });
    return map;
  }, [contacts, messages, currentUser.id]);
  ...
  const filteredSortedContacts = useMemo(() => {
    ...
    return contacts
      ...
      .sort((a, b) => {
        const tA = lastMessageTimestampByContact.get(a.id) || 0;
        const tB = lastMessageTimestampByContact.get(b.id) || 0;
        if (tB !== tA) return tB - tA;
        return a.fullName.localeCompare(b.fullName);
      });
  }, [contacts, searchTerm, lastMessageTimestampByContact]);

  const filteredSortedConversations = useMemo(() => {
    const query = searchTerm.trim().toLowerCase();
    const getPairLastTs = (pair: { u1: string; u2: string }) =>
      messages
        .filter(m =>
          (m.senderId === pair.u1 && m.receiverId === pair.u2) ||
          (m.senderId === pair.u2 && m.receiverId === pair.u1)
        )
        .map(m => new Date(m.timestamp).getTime())
        .sort((a, b) => b - a)[0] || 0;
    ...
    return activeConversations
      .filter(pair => {
        const { name1, name2, role1, role2 } = getParticipantNames(pair);
        if (!query) return true;
        return (
          name1.toLowerCase().includes(query) ||
          name2.toLowerCase().includes(query) ||
          role1.toLowerCase().includes(query) ||
          role2.toLowerCase().includes(query)
        );
      })
      .sort((a, b) => getPairLastTs(b) - getPairLastTs(a));
  }, [activeConversations, messages, searchTerm, users]);
  ...
  const handleSend = () => {
    ...
    addNotification(
      selectedContactId,
      'New Message',
      `${currentUser.fullName} sent you a message: "${messageText.slice(0, 80)}${messageText.length > 80 ? '...' : ''}"`,
      'MESSAGE',
      'messaging',
      { senderName: currentUser.fullName },
      { skipAdminBroadcast: true },
      { linkChatUserId: currentUser.id }
    );
    setInput('');
  };
  ...
  function getParticipantNames(pair: { u1: string, u2: string }) {
  ...
  }, [chatMessages, selectedContactId, monitorPair]);
    ...
    <div className="h-[calc(100vh-160px)] flex bg-card rounded-3xl border border-border shadow-xl overflow-hidden transition-all">
      ...
      <div className="w-80 border-r border-border flex flex-col bg-secondary/20">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide">
          {isMonitorMode && filteredSortedConversations.length > 0 && (
          ...
          {isMonitorMode && filteredSortedConversations.map((pair, idx) => {
      ...
      <div className="flex-1 flex flex-col bg-card transition-colors">
        {(selectedContact || (isMonitorMode && monitorPair)) ? (
          <>
            ...
            <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-8 space-y-6 bg-muted/10 transition-colors">
